...
class ODETerm(nn.Module):
    ...
    def init(self, problem: InitialValueProblem, stats: dict[str, Any]):
        if torch.jit.is_scripting():
            events: list[Any] = []
            stats["fw_model_times"] = events
        else:
            stats["fw_model_times"] = []
    ...
    def vf(
        ...
        if torch.jit.is_scripting():
            fw_model_times = stats["fw_model_times"]
            assert isinstance(fw_model_times, list[Any])
            start = torch.cuda.Event(enable_timing=True)
            end = torch.cuda.Event(enable_timing=True)
            fw_model_times.append(start)
            fw_model_times.append(end)
            ...
            with cuda_timing() as time:
                stats["fw_model_times"].append(time)
                if self.with_args:
