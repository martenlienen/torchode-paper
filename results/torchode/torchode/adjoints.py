...
class BacksolveFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_ts, grad_ys, grad_stats, grad_status):
        with torch.no_grad(), cuda_timing() as time:
            ...
            shapes, aug_state = flatten_tensors(
                ...
                grad_ys[:, -1],
                *[
                    torch.zeros_like(p).expand(batch_size, *p.shape)
...
class AugmentedDynamicsTerm(nn.Module):
        ...
        def vjp_single_sample(t_i, y_i, adj_y_i, arg_i):
            def wrapper(params, t_, y_):
                if self.term.with_args:
                    return self.func(params, self.buffers, t_, y_, arg_i)[0]
                else:
                    return self.func(params, self.buffers, t_, y_)[0]
    ...
    def init(self, problem: InitialValueProblem, stats: dict[str, torch.Tensor]):
        stats["bw_model_times"] = []

        return self.term.init(problem, stats)

    def vf(
        self, t: TimeTensor, y: DataTensor, stats: dict[str, torch.Tensor], args: Any
    ) -> DataTensor:
        with cuda_timing() as time:
            stats["bw_model_times"].append(time)
            shapes, args = args
