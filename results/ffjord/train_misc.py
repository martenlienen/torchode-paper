...
from lib.layers.odefunc import divergence_approx, divergence_bf
...
def collect_stats(model):
    ...
    def collect(module):
        if (
            isinstance(module, layers.CNF)
            and hasattr(module, "stats")
            and module.stats is not None
        ):
            stats.append(module.stats)

    model.apply(collect)

    keys = ["n_steps", "n_accepted"]
    forward = {}
    backward = {}
    for key in keys:
        for s in stats:
            if key in forward:
                ...
                forward[key] = s[key]
            if "backsolve" not in s:
                continue
            for bs in s["backsolve"]:
                if key in backward:
                    backward[key] += bs[key]
                else:
                    backward[key] = bs[key]
    ...
    for key in keys:
        if key in forward:
            forward[key] = forward[key].detach().cpu().numpy()
        if key in backward:
            backward[key] = backward[key].detach().cpu().numpy()

    return forward, backward


def collect_solver_and_model_times(model):
    fw_solver_times = []
    bw_solver_times = []
    fw_model_times = []
    bw_model_times = []
    ...
    def collect(module):
        if (
            ...
            fw_solver_times.append(module.stats["fw_solver_time"])
            if "bw_solver_time" in module.stats:
                bw_solver_times.append(module.stats["bw_solver_time"])
            fw_model_times.extend(module.stats["fw_model_times"])
            if "backsolve" in module.stats:
                for bw_stats in module.stats["backsolve"]:
                    if "bw_model_times" in bw_stats:
                        bw_model_times.extend(bw_stats["bw_model_times"])
            else:
                if "bw_model_times" in module.stats:
                    bw_model_times.extend(module.stats["bw_model_times"])

    model.apply(collect)

    sum_times = lambda ts: sum(t() for t in ts)
    fw_total_solver_time = sum_times(fw_solver_times)
    bw_total_solver_time = sum_times(bw_solver_times)
    fw_total_model_time = sum_times(fw_model_times)
    bw_total_model_time = sum_times(bw_model_times)

    return (
        fw_total_solver_time - fw_total_model_time,
        bw_total_solver_time - bw_total_model_time,
        fw_total_model_time,
        bw_total_model_time,
    )
...
def count_nfe(model):
    ...
    def count(module):
        if isinstance(module, layers.ODEfunc):
            ne.append(module.num_evals())

    model.apply(count)

    return len(ne), sum(ne)
