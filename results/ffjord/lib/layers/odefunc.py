...
class ODEfunc(nn.Module):
    ...
    def forward(self, t, states, e=None):
        assert len(states) >= 2
        y = states[0]
        ...
        with torch.set_grad_enabled(True):
            ...
            #         e = e[self.batch_idx:self.batch_idx+1]
            #     divergence = self.divergence_fn(dy, y, e=e).view(batchsize, 1)

            if e is None:
                e = self._e

            dy, vjp = functorch.vjp(self.diffeq, t, y, *states[2:])
            _, e_dzdx = vjp(e)
            e_dzdx_e = e_dzdx * e
            approx_tr_dzdx = e_dzdx_e.view(y.shape[0], -1).sum(dim=1, keepdim=True)
            divergence = approx_tr_dzdx
        if self.residual:
            dy = dy - y
            divergence -= torch.ones_like(divergence) * torch.tensor(np.prod(y.shape[1:]), dtype=torch.float32
                                                                     ).to(divergence)
        #return tuple([dy, -divergence] + [torch.zeros_like(s_).requires_grad_(True) for s_ in states[2:]])
        return tuple([dy, -divergence] + [torch.zeros_like(s_) for s_ in states[2:]])
